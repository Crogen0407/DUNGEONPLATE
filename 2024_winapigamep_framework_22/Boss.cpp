...
Boss::Boss() : _currentSkill{ nullptr }
{
	...
	GetComponent<Animator>()->CreateAnimation(L"Boss", texture, { 0.f, 0.f }, texSize, { (float)texSize.x, 0.f }, 6, 0.2f, false);
...
void Boss::Update()
{
	...
	if (_prevDash + _dashDelay < TIME)
	{
		...
		if (GetPosition().x < 100) dashDir = { 1,0 };
		else if (GetPosition().x > SCREEN_WIDTH - 100) dashDir = { -1,0 };
